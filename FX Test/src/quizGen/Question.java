...
public class Question {
    ...
    public void calcAnalyzedResponse(){
        ...
        Boot.GlobalS.updateQuestionsCompleted();    //update stating that a question was completed
        Boot.GlobalS.updateTotalScore();
        if(getResponse() == getAnswer()){
        	...
        	infoOnResponse = " * incorrect (answer was " + answer + ")";
        	correct = false;
        }
        
        analyzedResponse = response + infoOnResponse;
        Boot.GlobalS.refreshCorrectAnswerDisplayBox();
    }
    	//related to ^
    ...
    public void addCorrectToOperator(){
        Store_GlobalSetting gSettings = Boot.GlobalS;//.totalRight++;
    ...
    public Double getMaxValue(){
        ...
        if(symbol.equalsIgnoreCase(Symbols.PLUS)){
            return (double)Boot.operators.maxValues[0];
        }
        else if(symbol.equalsIgnoreCase(Symbols.MINUS)){
            return (double)Boot.operators.maxValues[1];
        }
        else if(symbol.equalsIgnoreCase(Symbols.TIMES)){
            return (double)Boot.operators.maxValues[2];
        }
        else if(symbol.equalsIgnoreCase(Symbols.DIVIDE)){
            return (double)Boot.operators.maxValues[3];
        }
    ...
    public Integer getMaxInputLength(){
        if(Boot.filter.findNumOfDec(getMaxValue()) == 0)
