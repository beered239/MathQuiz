...
* */
    ...
    public void genPt1(int[] range){
        pt1 = Boot.selectRange.withinArray(range); //((0 to 0.99) * (13)) + 0    --> 0-12       (note: already gets sorted in method.. but should still be optional)
    }
    ...
    public void genPt2(Store_Setting settingInfo, int[] range, int howMany){
        ...
        int practiceNum = Boot.filter.fixI(settingInfo.values.get(5));

        int index, value;
             if(difficultyI == 1){
            index = Boot.selectRange.chance(range.length,0.12,howMany);     //howMany: AS: 4, D: 5, M:6
            pt2 = range[index];
        }//easy mode
        else if(difficultyI == 2){
            pt2 = Boot.selectRange.withinArray(range); return;
        }//normal mode
        else if(difficultyI == 3){
            index = Boot.selectRange.chance(range.length,0.03, howMany);
            pt2 = range[index];
        }//hard mode
        else if(difficultyI == 4){
            pt2 = Boot.selectRange.within(pt2Range[0],pt2Range[1]);
        }//sandbox mode
