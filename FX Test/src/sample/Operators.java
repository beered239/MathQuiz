...
public class Operators {
    ...
    public static void getGlobalSData(){
            numOfQuestionsInOp = Main.GlobalS.getIntSettingA(0,3,true);
    ...
    public void createAllQuestions() {
        ...
        for(i=1; i<=numOfQuestionsInOp[0]; i++){
            addAddQuestion(Main.addition.generateQuestion());
        }//for addition
        for(i=1; i<=numOfQuestionsInOp[1]; i++){
            addSubQuestion(Main.subtraction.generateQuestion());
        }//for subtraction
        for(i=1; i<=numOfQuestionsInOp[2]; i++){
            addMulQuestion(Main.multiplication.generateQuestion());
        }//for multiplication
        for (i = 1; i <= numOfQuestionsInOp[3]; i++) {
            addDivQuestion(Main.division.generateQuestion());
        }//for division
    ...
    public void createQuiz(){
        ...
        for(int i=0; i< listSize; i++){
            r  = Main.selectRange.within(aSize+sSize+mSize+dSize   -1);
            if(r < aSize){
        ...
        //Main.dispQuiz.displayAllQuestion();
    }
    ...
    public void calcMaxInputs(){
        ...
        Store_Setting opInfo = Main.GlobalS;    //just to initialize (could use = null)
        int numOfQ;
        for(int i=0; i<numOfQuestionsInOp.length;i++){
            numOfQ = numOfQuestionsInOp[i];
            if(numOfQ>0){
                if(i==0)        opInfo = Main.AdditionS;
                else if(i==1)   opInfo = Main.SubtractionS;
                else if(i==2)   opInfo = Main.MultiplicationS;
                else if(i==3)   opInfo = Main.DivisionS;
                answerRangeLine = opInfo.values.get(4);			//MAKE SURE THIS GETS UPDATED WHEN Y GETS UPDATED 
