...
public class Store_Setting {
    ...
    //string arrays of acceptable terms
        //boolean terms:
        private String[] booleanDictionary = {"yes", "no", "true", "false", "y", "n", "t", "f"}; //single letter, ignore case
    ...
    public Boolean interpretValueB(String value){
        ...
        for(int i=0; i<boolTrueDict.length; i++){
            if(value.equalsIgnoreCase(boolTrueDict[i]) || valueI > 0)
                return true;
            else if(value.equalsIgnoreCase(boolFalseDict[i]) || valueI == 0 || filterNum.seeIfInvalid(valueI))
                return false;
        }
     ...
     * */
        private void correctInvalidValues(){

        if(lines.size() != settingTypeA.length)    {changeValuesToDefault();}
        ...
        for(int i=0; i<values.size(); i++){
            ...
            if(currentSettingType.equalsIgnoreCase("valueB")){
                ...
                if (valueIB == 0)                                   //note: valueIB can't be null if value is gibberish it will become 0
                    values.set(i, defaultSettingValuesA[i]);
                else
     ...
     * */
        private void correctInvalidNames(){
            if(lines.size() != defaultSettingNamesA.length) {changeNamesToDefault();}
